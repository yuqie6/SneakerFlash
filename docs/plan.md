...
3) 可观测性
   ...
   - 日志：slog+lumberjack，结构化日志，trace-id/uid 贯穿。
...
5) VIP 与优惠券（周期按月，成长值=累计实付）
   - 成长等级：按累计实付金额计算永久等级（配置阈值表）；无单独成长值字段。
   - 付费 VIP：可购买有限期等级，权益等级取“成长等级 vs 付费 VIP 有效期”最大值。
   - 优惠券：满减/折扣两种，按月周期发放，VIP 等级越高每月配额越多；普通用户可购买优惠券；优惠券当月有效，单订单仅可用一张。
   - 成长等级分层（示例，越往后差距加大，单位：元实付累计）：
     - L1：0-999
     - L2：1,000-4,999
     - L3：5,000-19,999
     - L4：20,000+

...
// 更新用户密码
func (r *UserRepo) UpdatePassword(ctx context.Context, uid uint, newPassword string) error {
	return r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", uid).Update("password", newPassword).Error
}

// 更新用户基础资料
func (r *UserRepo) UpdateProfile(ctx context.Context, uid uint, values map[string]any) error {
	if len(values) == 0 {
		return nil
	}
	return r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", uid).Updates(values).Error
}

// GetByIDForUpdate 查询并加行级锁，避免并发成长值更新丢失。
func (r *UserRepo) GetByIDForUpdate(ctx context.Context, id uint) (*model.User, error) {
	var user model.User
...
func (r *UserRepo) UpdateGrowth(ctx context.Context, userID uint, totalSpentCents int64, growthLevel int) error {
